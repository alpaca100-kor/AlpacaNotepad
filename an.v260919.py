...
class MemoStore:
    """memos.json / memos_calendar.json / settings.ini / quick_inputs.json /
    holidays.json 파일 입출력만 담당. tkinter 위젯을 전혀 참조하지 않으므로
    GUI 없이도 단위 테스트가 가능함. 저장할 데이터는 항상 인자로 받고
    (self가 데이터를 들고 있지 않음), 읽은 데이터는 검증해서 반환함."""
...
class GeneralMemoTab:
    ...
    def export_memos(self):
        ...
        try:
            if file_ext == ".json":
                self.app.save_memos()
                with open(self.app.store.file_path, 'r', encoding='utf-8') as f_in, open(filepath, 'w', encoding='utf-8') as f_out:
...
class CalendarMemoTab:
    ...
    def export_calendar_memos(self):
        ...
        try:
            if file_ext == ".json":
                self.app.save_calendar_memos()
                with open(self.app.store.calendar_file_path, 'r', encoding='utf-8') as f_in, open(filepath, 'w', encoding='utf-8') as f_out:
                    f_out.write(f_in.read())
            elif file_ext == ".txt":
                with open(filepath, "w", encoding="utf-8") as f:
...
class MemoApp:
    ...
    GeneralMemoTab / CalendarMemoTab에 위임함."""
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="일반메모 가져오기...", command=self.general_tab.import_memos)
        file_menu.add_command(label="일반메모 내보내기...", command=self.general_tab.export_memos)
        file_menu.add_separator()
        file_menu.add_command(label="달력메모 가져오기...", command=self.calendar_tab.import_calendar_memos)
        file_menu.add_command(label="달력메모 내보내기...", command=self.calendar_tab.export_calendar_memos)
        file_menu.add_separator()
    ...
    def copy_to_clipboard(self, event=None):
        ...
        if target is None:
            return "break"
        return self._copy_widget_text_to_clipboard(*target)
